...
server.listen(port, () => {
  console.log(`CatalogWise running at http://localhost:${port}`);
});
...
async function handleSubmissionsCsv(url, res) {
  ...
  try {
    ...
    res.writeHead(200, {
      ...
      "content-disposition": "attachment; filename=\"catalogwise-submissions.csv\""
    });
    res.end(csv);
  } catch {
    res.writeHead(200, {
  ...
  try {
    const response = await fetch(productsUrl, {
      headers: {
        ...
        "user-agent": "CatalogWise pre-MVP scanner"
      },
...
<html lang="en">
  <head>
    ...
    <title>CatalogWise Submissions</title>
    <style>
...
function generateFallbackReport({ storeUrl, products }) {
  ...
  return normalizeReport({
    ...
    summary: products.length
      ? `CatalogWise reviewed ${products.length} public product page${products.length > 1 ? "s" : ""} and found visible catalog issues.`
      : "CatalogWise could not read public Shopify product data from this URL, so this is a sample preview of the report format.",
    opportunities: [
      shortDescriptions > 0
...
function normalizeReport(report) {
  ...
  return {
    ...
    summary: String(report.summary || "CatalogWise found visible catalog cleanup opportunities."),
    opportunities,
    beforeAfterExamples,
    beforeAfter: beforeAfterExamples[0],
    bulkOpportunity:
